...
class Channel(BaseResourceObject):
    ...
    def __init__(self, **kwargs):
        ...
        self.permission_overwrites: Optional[List[Overwrite]]   = [Overwrite(**x) for x in kwargs.get('permission_overwrites')] if 'permission_overwrites' in kwargs and kwargs['permission_overwrites'] is not None else None
        ...
        self.recipients: Optional[List[User]]                   = [User(**x) for x in kwargs.get('recipients')] if 'recipients' in kwargs and kwargs['recipients'] is not None else None
        ...
        self.thread_metadata: Optional[ThreadMetadata]          = ThreadMetadata(**kwargs.get('thread_metadata')) if 'thread_metadata' in kwargs and kwargs['thread_metadata'] is not None else None
        self.member: Optional[ThreadMember]                     = ThreadMember(**kwargs.get('member')) if 'member' in kwargs and kwargs['meember'] is not None else None
        ...
        self.available_tags: Optional[List[ChannelTag]]         = [ChannelTag(**x) for x in kwargs.get('available_tags')] if 'available_tags' in kwargs and kwargs['available_tags'] is not None else None
        self.applied_tags: Optional[List[Snowflake]]            = kwargs.get('applied_tags', None)
        self.default_reaction_emoji: Optional[DefaultReaction]  = DefaultReaction(**kwargs.get('default_reaction_emoji')) if 'default_reaction_emoji' in kwargs and kwargs['default_reaction_emoji'] is not None else None
...
class ThreadListSync(BaseResourceObject):
    ...
    def __init__(self, **kwargs):
        self.guild_id       = kwargs.get('guild_id')
        self.channel_ids    = [x for x in kwargs.get('channel_ids')] if 'channel_ids' in kwargs and kwargs['channel_ids'] is not None else None
...
class ThreadMembersUpdate(BaseResourceObject):
    ...
    def __init__(self, **kwargs):
        ...
        self.added_members      = [ThreadMember(**x) for x in kwargs.get('added_members')] if 'added_members' in kwargs and kwargs['added_members'] is not None else None
        self.removed_member_ids = [x for x in kwargs.get('removed_member_ids')] if 'removed_member_ids' in kwargs and kwargs['removed_member_ids'] is not None else None